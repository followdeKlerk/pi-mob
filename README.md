...
Setup captures the owner's login-shell environment, copies the required executables into `~/.pi-mob`, writes a sanitized owner-only env file, installs the user LaunchAgent using the captured env, configures the bridge-owned private Tailscale Serve route, and starts the bridge. It guides you if Tailscale is missing, signed out, or missing MagicDNS; it does not silently provision anything.
...
- Work across trusted repositories with the bridge transporting Pi's normal execution model.

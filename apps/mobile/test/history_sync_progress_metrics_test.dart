...
void main() {
  ...
  test('history sync reports current chat, ETA, throughput, and bounded remaining', () async {
    ...
    try {
      ...
      expect(coordinator.historySyncCurrentSessionName, _firstSessionId.substring(0, 8));
      ...
      socket.server(_response('session.history.page.result', {
        'items': [
          {
      ...
      await _eventually(
        () => socket.sent.where((message) => message['type'] == 'session.history.page').length >= 2,
      );
...
Future<void> _eventually(bool Function() condition) async {
  ...
  while (!condition()) {
    if (DateTime.now().isAfter(deadline)) throw TestFailure('Condition was not met');

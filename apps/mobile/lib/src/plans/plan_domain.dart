...
@immutable
class PlanStepData {
  const PlanStepData({required this.stepId, required this.title, required this.status, this.blocker});
  final String stepId;
  final String title;
  final String status;
...
class PlanSnapshotData {
  ...
  static PlanSnapshotData? tryParse(Map<String, Object?> payload) {
    final planId = _boundedString(payload['planId'], maxLength: _maxPlanIdLength);
    ...
    final source = _boundedString(payload['source'], maxLength: _maxSourceLength);
    ...
    if (planId == null || revision == null || sessionId == null || turnId == null || source == null || staleValue is! bool || capability is! Map || stepsRaw is! List || stepsRaw.length > 64) {
      return null;
    }
    final cap = Map<String, Object?>.from(capability);
    if (cap['state'] != 'available') return null;
    if (cap['source'] is String && (cap['source'] as String).length > _maxSourceLength) return null;
    ...
    for (final item in stepsRaw) {
      ...
      final title = _boundedString(map['title'], maxLength: _maxStepTitleLength);
      final status = map['status'];
      if (stepId == null || title == null || status is! String || !_planStepStatuses.contains(status)) {
        return null;
      }
      final blocker = map['blocker'] == null ? null : _boundedString(map['blocker'], maxLength: _maxBlockerLength);
      steps.add(PlanStepData(stepId: stepId, title: title, status: status, blocker: blocker));
    }
...
class PlanUnavailableData {
  const PlanUnavailableData({required this.reason, required this.message, required this.remediation});
  final String reason;
  ...
  static PlanUnavailableData? tryParse(Map<String, Object?> payload) {
    ...
    if (state is! String || !_planUnavailableStates.contains(state)) return null;
    final reason = state == 'available' ? 'advertised' : state;
    if (state == 'available') {
      final message = map['reason'] is String ? map['reason'] as String : 'Plans are available';
      return PlanUnavailableData(reason: reason, message: message, remediation: map['remediation'] is String ? map['remediation'] as String : null);
    }
    ...
    return PlanUnavailableData(reason: reason, message: reasonStr, remediation: remediationStr);
  }
}
...
class PlanState {
  ...
  PlanState copyWith({PlanSnapshotData? snapshot, PlanUnavailableData? unavailable, bool? refreshing}) {
    return PlanState(
...
PlanState reducePlan(PlanState state, String type, Map<String, Object?> payload) {
    ...
    return snapshot != null
        ...
        : PlanState(
            ...
            unavailable: const PlanUnavailableData(
              ...
              remediation: 'Refresh after the host publishes a valid R2 plan snapshot.',
            ),
            refreshing: false,
          );
  }
  if (type == 'plan.unavailable') {
    ...
    return PlanState(
      snapshot: null,
      unavailable: unavailable ?? const PlanUnavailableData(reason: 'invalid_payload', message: 'Plan unavailable payload was invalid', remediation: 'Refresh after the host publishes a valid R2 plan unavailable payload.'),
      refreshing: false,
    );
  }
...
String? _boundedString(Object? value, {required int maxLength}) {
  if (value is! String || value.isEmpty || value.length > maxLength) return null;
  return value;
}

String? _revisionToken(Object? value) {
  return value is String && value.isNotEmpty && value.length <= 128 ? value : null;
}
...
String? _isoUtcString(Object? value) {
  if (value is! String) return null;
  return RegExp(r'^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}(?:\.[0-9]{3})?Z$').hasMatch(value) ? value : null;
}

...
class _GlobalSearchSheetState extends State<GlobalSearchSheet> {
  ...
  void _handleTap(SearchHit hit) {
    ...
    final kind = hit.destination['kind'];
    if (kind == null ||
        kind == 'transcript' ||
        kind == 'workspace' ||
        kind == 'git') {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text(
          'This search destination is stale. Refresh search and try again.',
        ),
      ),
    );
  }
...
class _ResultTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    // The source pill used to live in a Column inside `trailing` alongside
    // the copy IconButton. With a default 56-72px ListTile that Column
    // overflowed vertically (pill + 4px gap + 48px IconButton ≈ 76px).
    // The pill now sits in the title row and `trailing` is just the copy
    // action, so the row height stays bounded by Material's ListTile
    // minimum.
    return ListTile(

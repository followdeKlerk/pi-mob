...
- **`hello.accepted.capabilities` must be a superset of the client's `requiredCapabilities`.** The client's `requiredCapabilities` is a closed `CapabilitySchema` enum (`streams.v1`, `commands.v1`, `controller_leases.v1`, `attachments.v1`, `extension_dialogs.v1`, `notifications.v1`, `files.v1`, `contexts.v1`); the bridge's `hello.accepted.capabilities` is a free-form `Type.Array(Type.String())` and contains the literal surface identifiers the host actually implements. The bridge MUST advertise every literal the client listed as required, and any future client that makes a support literal required but finds the host missing it fails the handshake with `unsupported_capability` (D-037 rule). Generator-owned fixtures therefore pair every hello scenario with the exact superset the test client expects — forgetting a required literal (e.g. advertising only `runtime.processes.v1` while the client requires `streams.v1` + `commands.v1`) silently ships a handshake that fails the first real client. (R5 fix path: 95bef9e added `streams.v1` + `commands.v1` to `response-hello-accepted-valid.json`, `scenario-hello-accept.json`, and `scenario-hello-generation-changed.json` so the generator and corpus match the client's required set.)

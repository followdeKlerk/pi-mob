...
- **TypeBox/JSON-Schema `Optional` means the key may be absent — it does NOT mean the value may be explicitly `null`.** A field declared as `Type.Optional(X)` only permits the key to be missing; an explicit `null` is rejected unless the schema is `Type.Union([X, Type.Null()])`. The Dart `protocol_fixture.dart` R1/R2 validators (`planTarget`, `providerSummary`, `truncation`, `errorInfo`, `blocker`, `digest`, `errorCode`, etc.) must therefore branch on `payload.containsKey(field)` (presence), never on `payload[field] != null` (truthy) — the latter silently accepts `{planTarget: null}` and lets an idempotency retry retarget a prompt with a missing plan. (Fix landed in 0e95e8c: `if (payload['planTarget'] != null)` → `if (payload.containsKey('planTarget'))` plus the `rejects explicit null` test in `apps/mobile/test/protocol_fixture_test.dart`.) The inverse trap is treating `null` as required: `command.state.errorCode` MUST be present-and-nullable, and `_validateCommandPayload` enforces that with `!containsKey` → throw before the null-arm runs the `_oneOf` enum check. Wire this rule into every validator you add: pick the right optionality model first, then code the gate.

...
- **Hello capability negotiation is asymmetric: current optional surfaces stay optional, while acceptance is a required-capability superset.** The request's closed `CapabilitySchema` permits both baseline and additive surface literals, so schema validity alone does not prove negotiation policy: the current mobile client lists `files.v1`, `contexts.v1`, and `runtime.processes.v1` in `optionalCapabilities`, never `requiredCapabilities`; the bridge advertises each only when implemented, and absence maps to explicit unavailable UI. Independently, the free-form `hello.accepted.capabilities` MUST contain every literal that request did list in `requiredCapabilities`, or the handshake fails with `unsupported_capability`. Generator-owned fixtures must prove both halves together: the R5 request requires `streams.v1` + `commands.v1` and optionally requests `runtime.processes.v1`; its accepted response advertises all three because the fixture host implements R5. (Current R5 repair path: 95bef9e restored the required-capability superset in the three generated accepted-response/scenario fixtures; bb2088a moved `runtime.processes.v1` from required to optional in `hello-valid.json` and locked both lists with assertions.)
- **Durable context target parity follows [`D-038`](docs/DECISIONS.md#d-038--exact-discriminant-for-durable-context-mutation-targets).** A context mutation target is exactly one closed shape: `{ kind: "file", path, ranges?, revision? }`, `{ kind: "source", sourceId, revision? }`, or `{ kind: "all" }`. `kind` is never inferred from `path` or `sourceId`, never normalized before semantic hashing, and is required by TypeScript, generated schema, fixtures, bridge, and Dart alike. The generated valid `context.unpin` fixture therefore carries `kind: "file"`; a missing-kind fixture must fail and be repaired only by adding that literal.
...
- **TypeBox/JSON-Schema `Optional` means the key may be absent — it does NOT mean the value may be explicitly `null`.** A field declared as `Type.Optional(X)` only permits the key to be missing; an explicit `null` is rejected unless the schema is `Type.Union([X, Type.Null()])`. The Dart `protocol_fixture.dart` R1/R2 validators (`planTarget`, `providerSummary`, `truncation`, `errorInfo`, `blocker`, `digest`, `errorCode`, etc.) must therefore branch on `payload.containsKey(field)` (presence), never on `payload[field] != null` (truthy) — the latter silently accepts `{planTarget: null}` and lets an idempotency retry retarget a prompt with a missing plan. (Fix landed in 4d8a46d: `if (payload['planTarget'] != null)` → `if (payload.containsKey('planTarget'))` plus the `rejects explicit null` test in `apps/mobile/test/protocol_fixture_test.dart`.) The inverse trap is treating `null` as required: `command.state.errorCode` MUST be present-and-nullable, and `_validateCommandPayload` enforces that with `!containsKey` → throw before the null-arm runs the `_oneOf` enum check. Wire this rule into every validator you add: pick the right optionality model first, then code the gate.
- **Closed nested parity must recurse.** `_closedObject` is not a one-shot root-level check — every nested shape (step → timing, plan.snapshot → capability → status, recipe.activity → providerSummary → truncation, ErrorInfo → sub-envelopes) must close its own `additionalProperties: false` against the canonical key set, or a sibling at depth N+1 reopens depth N. The same recurse-or-leak applies to the bounded-length checks: a 240-char cap on `step.blocker` is meaningless if the bridge can replace it with `{blocker: {value: "x".repeat(100000)}}`. When you add a new envelope, walk the schema tree and close every object; the generator-owned fixture corpus and `bun run fixtures:check` are the cheapest proof that closed parity actually recursed.
...
- **Invalid-fixture proofs must hard-code expected outcomes; fixture metadata is never the oracle.** A semantic-invalid fixture's own `semanticExpectation.{outcome,errorCode,currentRevision,maxCombinedItems}` describes what the fixture CLAIMS to violate — using those fields as the `expect()` target (`const expected = fixture.semanticExpectation?.errorCode; expect(validator(...) ).toBe(expected)`) is circular and silently passes when the validator returns the wrong code. The test code itself must be the oracle: hard-code the expected error code and revision per fixture filename (e.g. `isStaleRevision ? "file_stale" : "invalid_message"`), and either drop the metadata assertions entirely or restrict them to the "fixture labels itself correctly" invariant. The hard-coded assertion is the only thing that catches a validator that returns `undefined`, returns the wrong code, or returns the right code for the wrong reason. (D-037 fix path: 3e5d84f introduced `validatePromptSemantics` + repair; 0151e4d replaced `fixture.semanticExpectation?.errorCode` with a hard-coded ternary per filename, so the test now fails if the validator's branch logic drifts.)
- **Pair every invalid fixture with a targeted single-invariant repair.** For each schema-invalid or semantic-invalid fixture, mutate exactly one field that should flip it to valid (e.g. bump `fileRefs[0].revision` to the current revision, drop one `attachmentId` to bring the joint budget to 4, restore the deleted `expectedRevision`) and assert the schema validator passes on the repair. The repair proves the fixture fails ONLY for the intended invariant — if the repair still fails the schema, the fixture violated an incidental sibling and the proof is invalid; if the repair validates but a one-line tweak elsewhere would also pass, the intended invariant is not isolated. The repair must be visible in the test source, not buried in the generator, so reviewers can see what single field is the failing invariant. Use `LIMITS.maxTreeDepth` / `LIMITS.maxFileSize` / `LIMITS.maxTreePageItems` as the canonical ceilings when repairing oversize fixtures so the boundary stays in lock-step with the schema.
...
- **Protocol `type` names are family-scoped, not globally unique.** A control and an event may intentionally share the same `type`; classify the envelope by its kind/identity first, then validate or dispatch through that family's registry. Never concatenate registries and use first-match/order as the discriminator: registry ordering is not wire identity and can silently route a same-name control through the event schema (or vice versa). Fixture parity checks and mobile decoders must preserve the envelope discriminator before looking up `type`.

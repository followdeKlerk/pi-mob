...
Define bounded, additive event/control/response families for recipe activity, plans/step targets, files and attachments, context sources/mutations, registered processes, Git/CI, attention, agents/actions, commands/skills/MCP, and global-search destinations. Every family carries opaque IDs, revision/page tokens, limits, capability status, typed errors, lease/valid-state requirements, and forward-compatible optional events. Per [D-037](DECISIONS.md#d-037--f0-files-and-context-protocol-topology), `files.v1` and `contexts.v1` are independent optional hello capabilities; R3 invalidations use the mandatory host stream with `workspaceId`, never a workspace stream class; and only reads/pages are controls. `prompt.submit.attachmentIds` plus `fileRefs` share the four-item budget, while R4 pin/unpin/exclude/refresh are durable session commands with normal command ID, lease, idempotency, and outcome semantics. Add valid, invalid, oversized, stale, unavailable, combined-cap, hello-capability, stream-ownership, and mutation-recovery fixtures in `packages/protocol-fixtures/`; generate schemas; do not enable an exclusion.
...
Add root-confined bridge controls for lazy repository tree, filename search, content search, metadata/modified indicators, and paginated UTF-8 line reads. Reject canonicalization/symlink/`..` escapes. R3 invalidation events are on the mandatory host stream and include `workspaceId`; do not add a workspace stream/subscription/cursor class. Mobile provides recent files, modified markers, syntax-highlighted bounded viewing, line-number/range selection, copy path or selected text, attach file/selected lines to the composer, refresh, and visible pagination/truncation. File references carry workspace/path/range/digest/revision, are revalidated at send, and share the existing four-item prompt-context budget with binary `attachmentIds`. Never eagerly scan or mirror the repository and never edit, diff, preview, or implicitly change context.

**Acceptance/tests:** large/binary/denied/deleted/stale files and unsupported languages have explicit states; attachments carry workspace/path/range/digest/revision and are revalidated at send. Test traversal, bounds/page tokens, UTF-8 boundaries, search, host-stream `workspaceId` invalidation, recents, range selection, copy/attach, combined binary/file fifth-item rejection, stale attachments, and one sheet flow.
...
Keep a dedicated inspector, not a browser tab, showing active model and thinking level, workspace instructions, pinned files and selected ranges, context/token usage, compacted state, and unavailable/stale sources with revision/last refresh. `context.snapshot.request` is a read control; pin, unpin, exclude, and refresh are authoritative durable **session commands** with command IDs, lease/revision checks, idempotency, and replayable outcomes. Compact and the existing model picker reuse their existing durable command contracts. Context state is session-stream scoped; `contexts.v1` is independently optional, and browsing a file never pins it.

**Acceptance/tests:** pin/unpin/exclude appear only after host command acceptance and the authoritative session snapshot; unknown usage is unavailable, not zero; stale state cannot look current; reconnect/restart reconstructs state; unavailable source/action has a reason. Cover optional-capability hello fixtures, command duplicate/conflict/lease/stale/indeterminate recovery, model picker/compact reuse, and browser-versus-context widget isolation.

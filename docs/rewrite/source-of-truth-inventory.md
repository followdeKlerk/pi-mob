...
| Pi JSONL session history | `external-history.ts` + Pi-owned `*.jsonl` | Import/recovery input only | Mobile reducer parity test proves `recipe.activity` snapshots are replayable from canonical events; delete only when those snapshots stop carrying non-canonical fields | Bridge external-history | Phase 7 |
...
| `recipe.activity` events on the session stream | `DurableRecipeActivityProjection.append()` | Derived snapshot retained for parity | Remove only after mobile reducer parity proves the canonical reducer can replay the same projection from canonical events | Bridge recipe-activity | Phase 7 |
...
| Legacy stream plus `CanonicalSessionStore` dual write | Bridge runtime | Compatibility output for operational/older clients and recipe projection | Remove after older-host support and recipe projection migration are explicitly retired | Compatibility phase |
...
- `session_events.v2` is implemented and exercised by the normal daemon
  and coordinator, but the legacy transcript paths remain during migration.

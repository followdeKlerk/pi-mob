...
export class SupervisedRpcClient {
  ...
  private project(value: unknown): void {
    ...
    const record = value as { type?: unknown; method?: unknown; isTerminal?: unknown };
    ...
    try {
      if (type === "agent_start" && state === "idle") this.supervisor.transition(this.processId, "running");
        ...
        (type === "agent_settled" || (type === "agent_end" && record.isTerminal === true)) &&
        ["running", "waiting_for_input", "compacting"].includes(state)
      ) {

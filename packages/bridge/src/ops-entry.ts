...
export async function main(argv: readonly string[]): Promise<number> {
  ...
  if (paths && home) {
    ...
    setupDefaults = {
      ...
      sourceExtensionPath: resolve(executableDir, "..", "extensions", "pi-mob-extension.js"),
      bridgeVersion: process.env.PI_MOB_BRIDGE_VERSION ?? "0.0.0",
      protocolVersion: "1.0",
      pathDirs: [executableDir, "/opt/homebrew/bin", "/usr/local/bin", "/usr/bin", "/bin", "/usr/sbin", "/sbin"],
      port: 8788,
    };
  }

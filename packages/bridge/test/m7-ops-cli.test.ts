...
function setupDefaults(root: string): SetupDefaults {
  return {
    ...
    piSessionDir: join(root, "sessions"), sourceExtensionPath: join(root, "extension.ts"),
    bridgeVersion: "0.1.0", protocolVersion: "1.0", pathDirs: ["/usr/bin"], port: 9443,
  };
}
...
describe("M7 operations CLI", () => {
  ...
  test("setup installs with workspace-centered defaults only after Tailscale and MagicDNS are ready", async () => {
    ...
    for (const file of [defaults.piExecutable!, defaults.sourceCliExecutable, defaults.sourceBridgeExecutable, defaults.sourceExtensionPath]) writeFileSync(file, "fixture");
    ...
    expect(createNodeFileSystemPort().readFile(installedExtension).toString()).toBe("fixture");
    ...
    expect(installedPlist).toContain(installedExtension);

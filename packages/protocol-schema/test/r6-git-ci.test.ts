...
test("R6 Git summaries close fields and enforce identifiers, counts, URLs, checks, and actions", () => {
	...
	const failedChecks = Array.from(
		{ length: LIMITS.maxFailedChecks + 1 },
		(_, index) => ({
			name: `check-${index}`,
			status: "failure",
		}),
	...
	const invalid: unknown[] = [
		...
		gitSummary({
			latestCommit: { ...gitSummary().latestCommit, sha: "not-a-sha" },
		}),
		gitSummary({
			latestCommit: { ...gitSummary().latestCommit, url: undefined },
		}),
		gitSummary({
			latestCommit: { ...gitSummary().latestCommit, private: "leak" },
		}),
		gitSummary({
			pullRequest: { number: 42, title: "PR", url: "http://example.test/42" },
		}),
		gitSummary({
			pullRequest: {
				number: 42,
				title: "PR",
				url: `https://example.test/${"x".repeat(LIMITS.maxExternalUrlLength)}`,
			},
		}),
		gitSummary({
			pullRequest: {
			...
			failedChecks: [
				{
					name: "check",
					status: "failure",
					logSummary: "x".repeat(LIMITS.maxLogSummaryLength + 1),
				},
			],
		}),
		...
		gitSummary({
			failedChecks: { totalCount: LIMITS.maxFailedChecks + 1, items: [] },
		}),
		gitSummary({
			failedChecks: { totalCount: LIMITS.maxFailedChecks, items: failedChecks },
		}),
		gitSummary({
			failedChecks: {
				totalCount: 1,
				items: [{ name: "check", status: "failure", private: "leak" }],
			},
		}),
...
test("R6 Git summary and unavailable events are closed host-stream records", () => {
	...
	for (const [type, payload] of Object.entries(payloads)) {
		expect(
			EVENT_STREAM_OWNERSHIP[type as keyof typeof EVENT_STREAM_OWNERSHIP],
		).toBe("host");
...
test("R6 Git actions are durable session commands with lease, revision, confirmation, metadata, and stable errors", () => {
	...
	for (const type of ["git.commit.request", "git.push.request"] as const) {
		...
		expect(
			commands.Check({
				...message,
				payload: gitCommandPayload({ sessionId: undefined }),
			}),
		).toBe(false);
		expect(
			commands.Check({
				...message,
				payload: gitCommandPayload({ sessionId: "not-a-uuid" }),
			}),
		).toBe(false);
		expect(
			commands.Check({
				...message,
				payload: gitCommandPayload({ workspaceId: undefined }),
			}),
		).toBe(false);
		expect(
			commands.Check({
				...message,
				payload: gitCommandPayload({ expectedRevision: undefined }),
			}),
		).toBe(false);
		expect(
			commands.Check({
				...message,
				payload: gitCommandPayload({ confirmation: undefined }),
			}),
		).toBe(false);
		expect(
			commands.Check({
				...message,
				payload: gitCommandPayload({
					confirmation: { confirmationId: "bad id" },
				}),
			}),
		).toBe(false);
		...
		expect(
			COMMAND_METADATA.find((entry) => entry.type === type)?.stableErrors,
		).toEqual([
			"invalid_message",
